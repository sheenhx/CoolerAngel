...
m = SimpleForm("network", translate("AP Client Join Network: Settings"))
...
function newnet.parse(self, section)
	local value = 0
	local x = uci.cursor()
	local ssid = m.hidden.join
	x:set("wireless","sta","disabled",value)
	x:set("wireless","sta","ssid",ssid)
	...
	if m.hidden.wep == "1" then
		...
		x:set("wireless","sta","key1",wconf.key1)
	...
	x:set("wireless","sta","encryption",wconf.encryption)
	x:set("wireless","sta","key",wconf.key)
